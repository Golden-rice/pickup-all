...
const _assgin = Object.assign ? Object.assign : extend
const PEACT_ELEMENT_TYPE = Symbol("PEACT_ELEMENT_TYPE")

// 校验是否是合法元素，只需要校验类型，重点是判断.$$typeof属性
function isValidElement(object) {
  return typeof object === 'object' && object !== null && object.$$typeof === PEACT_ELEMENT_TYPE;
}
...
class PeactCompositeComponentWrapper {
  ...
  receiveComponent(nextElement, newState){
    ...
    const nextState = _assgin(state, newState)
    ...
    // 比较 element 决定是否要更新
    if (_shouldUpdatePeactComponent(prevElement, nextElement)) {
      // 遍历子节点
...
const PeactClass = {
  createClass(sepc) {
    ...
    ElementClass.prototype = _assgin(ElementClass.prototype, sepc)
...
const Peact = {
  ...
  createElement(type, config, children) {
    ...
    function Element(type, key, props) {
      this.$$typeof = PEACT_ELEMENT_TYPE
      this.type = type
      this.key = key;
      this.props = props;
    }
